describe('TodoList', function () {
  ...
  context('卡片', function () {
    ...
    it('清除已完成卡片', function () {
      cy.get('.el-button > .el-icon-delete').click().wait(500)
        .get('.el-message-box__btns > .el-button--primary').click()
    })
    it('新增卡片', function () {
      cy.get('.function-row > .el-col > .el-button').click().wait(500)
        .get('.el-form-item__content > .el-input > .el-input__inner')
        .type('測試卡片')
        .wait(500)
        .get('.el-textarea__inner')
        .type('# Illa dumque cessit planxitque submisit fero quid')
    })
  })
})